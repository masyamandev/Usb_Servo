...
static int usbhidGetStringAscii(usb_dev_handle *dev, int index, char *buf,
		int buflen) {
	char buffer[256];
	int rval, i;
...
int usbhidOpenDevice(usbDevice_t **device, int vendor, char *vendorName,
		int product, char *productName, int _usesReportIDs) {
	struct usb_bus *bus;
...
int usbhidSetReport(usbDevice_t *device, char *buffer, int len) {
	...
	bytesSent = usb_control_msg((void *) device,
			...
			USB_HID_REPORT_TYPE_FEATURE << 8 | (reportId & 0xff), 0, buffer,
			len, 5000);
	if (bytesSent != len) {
		if (bytesSent < 0)
...
int usbhidGetReport(usbDevice_t *device, int reportNumber, char *buffer,
	...
	bytesReceived = usb_control_msg((void *) device,
			...
			USB_HID_REPORT_TYPE_FEATURE << 8 | reportNumber, 0, buffer, maxLen,
			5000);
	if (bytesReceived < 0) {
		fprintf(stderr, "Error sending message: %s\n", usb_strerror());
		return USBOPEN_ERR_IO;
